...
from externalResnet import resnet34, ResNet
...
def fillSUFB(model, train_loader):
    ...
    with torch.no_grad():
        for i, (images, labels, idx) in enumerate(train_loader):
            ...
            # Forward pass
            outputs = model((images, idx)).to('cpu')
            bank[idx,:] = outputs
...
def train_loop(phase):
    for i, (images, labels, idx) in enumerate(train_loader):
        ...
        if phase == 2:
            m = model.getSubBatchSize()
            idx2 = idx[0:m]
            # use list to index FB
            features = bank[idx]
            # call method in resnet code
            features = features.to(device)
            model.storeFeatures(features)
        ...
        if phase==2:
            tmp = model.retrieveFeatures().to('cpu')
            bank[idx2] = tmp[0:m,:]
